...
class PairDataset(data.Dataset):
    def __init__(self, root_dir, train=True, data_augmentation=True):
        ...
        self.image_list = []
        self.mask_list = []
        for dataset in ['DUTS', 'MSRA10K', 'SOC6K', 'XPIE']:
            temp_image_list = os.listdir(os.path.join(root_dir, dataset, 'images'))
            temp_mask_list = os.listdir(os.path.join(root_dir, dataset, 'masks'))
            temp_image_list = [dataset+'/images/'+l for l in temp_image_list]
            temp_mask_list = [dataset+'/masks/'+l for l in temp_mask_list]
            self.image_list += temp_image_list
            self.mask_list += temp_mask_list
    ...
    def __getitem__(self, item):
        img_name = os.path.join(self.root_dir, self.image_list[item])
        mask_name = os.path.join(self.root_dir, self.mask_list[item])
        img = Image.open(img_name)
        mask = Image.open(mask_name)
        img = img.convert('RGB')
        mask = mask.convert('L')
...
class CustomDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        sample = self.transform(sample)
        return sample, img_name.split('/')[-1]
